...
public class Reindeer implements Runnable {
	...
	/**
	 * Santa will call this function when it's time to ready the
	 * sleigh
	 * @param state
	 */
	...
	public void run() {
		...
		case AT_WARMING_SHED: 
			...
			if(scenario.reindeer.tryAcquire())
			{
				synchronized(scenario.lockReindeer)
				{
					try
					{
						scenario.lockReindeer.wait();
					}
					catch(InterruptedException e)
					{
						e.printStackTrace();
	...
	//to release the semaphores since they are not being used anymore
	public void goToSleigh()
	{
		scenario.reindeer.release();
	}
}
